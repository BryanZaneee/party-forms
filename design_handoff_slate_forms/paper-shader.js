...
(function () {
  ...
  class PaperShader extends HTMLElement {
    ...
    async connectedCallback() {
      ...
      this._fallback();
      try {
        const m = await import('https://esm.sh/@paper-design/shaders');
        this._m = m;
        if (!this.isConnected) return;
        const back = this.getAttribute('color-back') || '#070d1a';
        const front = this.getAttribute('color-front') || '#2447ff';
        const uniforms = {
      ...
      } catch (e) { /* fallback gradient already applied */ }
    }
    _speed() { return parseFloat(this.getAttribute('speed') || '0.4'); }
    _fallback() {
      this.style.background = 'linear-gradient(115deg,#070d1a 0%,#12203c 45%,#2447ff 90%,#19c8ff 120%)';
    }
    ...
    disconnectedCallback() { try { this._mount && this._mount.dispose(); } catch (e) {} this._mount = null; }
  }
